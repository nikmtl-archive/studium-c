...
//
// x TO DO 1: Write a function that calculates the Ackermann function according to the definition above
//          Test the program with very small values first!
// x TO DO 2: Extend the  function such that it counts the number of functions calls(use a global variable counter)
//          Print the function arguments and current count every x function calls. 
// x TO DO 3: Count the amount of recursively called functions (which are not yet completed) and print them as well
// x TO DO 4: Use static variables in ackermann for counting rather than global variables.
// x TO Dars of ascii-characters: xO 5: Optional: Visualize the current recursion depth with bxxxxxxxxxxxxxxxxx
